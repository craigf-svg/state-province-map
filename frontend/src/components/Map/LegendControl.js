...
import { getDensityColor, getLandmassColor, getLegendTextColor } from './MapStyleUtils'
...
const LegendControl = ({ densityOrLandmass }) => {
  ...
  const legend = useMemo(() => {
    ...
    const list = grades.map((x, index) => {
      return <div
        ...
        style={{
          background: getGradeColor(x), color: getLegendTextColor(getGradeColor(x))
        }} key={index}>
